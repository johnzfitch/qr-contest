...
#!POPCORN gpu B200
"""Blocked WY-Householder QR for B200 (qr_v2) — TF32 TRAILING PROBE.

IDENTICAL to submission_full.py except torch.backends.cuda.matmul.allow_tf32 = True.
Goal: the scalar fp32 panel_geqrt kernel is unchanged; only the BLAS-3 compact-WY
trailing update (the n^3 bulk, three einsums) is allowed to run on TF32 tensor cores
instead of fp32 CUDA cores. Probe #4 in the panel-attack series:
  fp64 panel ~100ms -> fp32 panel ~88ms -> on-device panel_geqrt ~13ms (fp32 CUDA-core
  trailing) -> THIS (TF32 tensor-core trailing) -> ?
Orthogonality is structurally free (Q from reflectors), so TF32 can only move the
factor residual; the gate is loose (~1.2e-3 at n=512) and the WY update is backward-
stable (additive error, no kappa^2), so the prior is it clears 19/19. Measured, not
assumed -- the leaderboard Testing stage gates correctness before timing.

Per panel [p0:pe] (width nb), one CUDA block per matrix:
  (1) panel_geqrt kernel: unblocked geqr2 on the tall panel B[p0:n, p0:pe] resident in
      shared memory -> Householder vectors below the diagonal, R on/above, tau[p0:pe], and
      the nb x nb compact-WY block coefficient T (DLARFT).
  (2) trailing update (cuBLAS, BLAS-3, NOW TF32 tensor-core):  C = B[p0:n, pe:n]
          C -= V (Tᵀ (Vᵀ C))          # apply (I - V Tᵀ Vᵀ) to the trailing columns
Output H = reflectors below + R above, tau — exactly torch.geqrf's layout.

n >= 2048 (benchmark batch 8 / 2) starves one-block-per-matrix occupancy, so those route to
torch.geqrf, which wins at that shape. Everything n <= 1024 uses the Householder spine.
"""
import torch
from torch.utils.cpp_extension import load_inline
from task import input_t, output_t

torch.backends.cuda.matmul.allow_tf32 = True          # <-- the only change vs submission_full.py
...
def _hh_qr(A):
    ...
    while p0 < n:
        ...
        if pe < n:                                    # (2) WY trailing update (BLAS-3, TF32)
