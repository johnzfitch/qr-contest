...
_K = load_inline(name="kqr_routed_sub", cpp_sources=[CPP_SRC], cuda_sources=[CUDA_SRC],
                 functions=["qr_blocked"], extra_cuda_cflags=["-O3"], verbose=False)

GEQRF_N = 4096          # only n>=4096 (b2) routes to geqrf; n=2048 (b8) fused wins 1.65x
SMEM_FLOATS = 56 * 1024
...
def custom_kernel(data: input_t) -> output_t:
    ...
    Ac = A.contiguous()
    if n == 512 or n >= 1536:        # fused wins at 512 and at batch-starved large-n (2048)
        H, tau = _K.qr_blocked(Ac, _fit_nb_fused(n), 0)
    else:                            # wavefront wins at 176/352/1024
