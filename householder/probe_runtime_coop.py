...
/* RUNTIME-USAGE probe: real cooperative launch + grid-wide barrier, self-verifying. */
__global__ void coop_probe_kernel(int* flag, int nblocks) {
    cg::grid_group grid = cg::this_grid();
    if (threadIdx.x == 0) atomicAdd(&flag[0], 1);
    grid.sync();                                   /* grid-wide barrier across all CTAs */
    if (blockIdx.x == 0 && threadIdx.x == 0)
        flag[1] = (flag[0] == nblocks) ? 1 : 0;    /* did the barrier see every block? */
}
int coop_runtime_check() {
    cudaDeviceProp prop; cudaGetDeviceProperties(&prop, 0);
    int mbps = 0;
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&mbps, (void*)coop_probe_kernel, 32, 0);
    int nblocks = prop.multiProcessorCount * (mbps > 0 ? mbps : 1);
    if (nblocks > 4096) nblocks = 4096;
    int* d; cudaMalloc(&d, 2*sizeof(int)); cudaMemset(d, 0, 2*sizeof(int));
    void* args[] = { (void*)&d, (void*)&nblocks };
    dim3 g(nblocks), b(32);
    cudaError_t e  = cudaLaunchCooperativeKernel((void*)coop_probe_kernel, g, b, args, 0, 0);
    cudaError_t e2 = cudaDeviceSynchronize();
    int h[2] = {0,0}; cudaMemcpy(h, d, 2*sizeof(int), cudaMemcpyDeviceToHost); cudaFree(d);
    if (e != cudaSuccess || e2 != cudaSuccess) return -1;
    return h[1];
}
...
_K = load_inline(name="kqr_fused_v2_sub", cpp_sources=[CPP_SRC], cuda_sources=[CUDA_SRC],
                 functions=["qr_blocked", "coop_runtime_check"], extra_cuda_cflags=["-O3"], verbose=False)
...
def custom_kernel(data: input_t) -> output_t:
    A = data
    if _COOP["v"] is None:
        _COOP["v"] = _K.coop_runtime_check()
        assert _COOP["v"] == 1, "coop grid.sync runtime check returned %s" % _COOP["v"]
    n = A.shape[-1]
    if n >= GEQRF_N:
