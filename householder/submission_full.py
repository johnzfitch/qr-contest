...
#!POPCORN gpu B200
"""Blocked WY-Householder QR for B200 (qr_v2).

QR the direct way: a blocked Householder factorization that emits the geqrf (H, tau)
contract with no reconstruction, no Gram/Cholesky, no orthogonality gate, and no fallback
on the fast path. Householder is unconditionally backward-stable, so the rank-deficient /
clustered / near-rank stress batches pass natively in FP32 — the cases that previously cost
a 80x-slow geqrf fallback.
...
Per panel [p0:pe] (width nb), one CUDA block per matrix:
  ...
  (2) trailing update (cuBLAS, BLAS-3, tensor-core):  C = B[p0:n, pe:n]
